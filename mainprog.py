...
def youtube2mp3 (url,outdir,fname,Token):
    ...
    if new_file.exists():
        ...
        subprocess.run(['python','-m',"spleeter", "separate", fname ,"-p" "spleeter:5stems", "-c", "wav", "-o", 'dehors'], capture_output=True)
        subprocess.run(["spleeter", "separate", fname ,"-p" "spleeter:5stems", "-c", "wav", "-o", 'dehors'], capture_output=True)
